...
CPPFLAGS += -Isrc -isystem ${SRC_DIR}/third-party -isystem ${SRC_DIR}/third-party/asio $(DEFINES)
...
ifeq ($(TARGETSYSTEM),WINDOWS)
  LDFLAGS += -lgdi32 -lwinmm -limm32 -lole32 -loleaut32 -lversion -lWindowsApp -lmincore -lBcrypt -liphlpapi
  ifeq ($(BACKTRACE),1)
    LDFLAGS += -ldbghelp
  endif
endif
...
version:
	@( VERSION_STRING=$(VERSION) ; \
        [ -e ".git" ] && \
          ...
          DIRTYFLAG=$$( [ -z "$$(git -c core.autocrlf=input -c core.safecrlf=false diff --numstat | grep -v -e lang/po/ -e src/lang_stats.inc)" ] || echo "-dirty.$$(date +%H%M%S)") && \
